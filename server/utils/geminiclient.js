...
async function generateVibeCheck(itineraryText) {
  try {
    const message = new HumanMessage(
      `Here's an itinerary:\n\n${itineraryText}\n\nGive me 5 short, funny, Gen-Z-coded vibe check prompts based on this trip. 
      Keep each vibe check to one phrase, max 5-7 words. These prompts are actually for a travel app where friends can 
      post short videos based on a prompt and their trip itinerary, so it's meant to store memories. 
      Rephrase these prompts into categories people can cache different memories into (eg funny laugh till you cry, 
      food, romantic, silly epic fails, awkward moments, pure joy, etc). Don't give explanations. 
      no [phrase]:[prompt] format. maybe include some meme references, or some references to popular media. Label them 1 to 5.`,
    );
