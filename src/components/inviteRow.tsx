...
const responses = [
    { label: "Going", emoji: "✅", gradient: ['#60a5fa', '#c084fc'] as [string, string] },
    { label: "Maybe", emoji: "🤔", gradient: ['#a78bfa', '#f472b6'] as [string, string] },
    { label: "Not Going", emoji: "❌", gradient: ['#f472b6', '#ec4899'] as [string, string] },
];
...
export default function InviteResponseBar({
  ...
  return (
        ...
        {responses.map((r) => (
            <Pressable
                key={r.label}
                onPress={() => onSelect(r.label.toLowerCase().replace(" ", "_") as any)}
                style={[
