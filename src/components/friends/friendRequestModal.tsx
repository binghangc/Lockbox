...
export default function FriendRequestModal({
  ...
  const handleFriendRequest = async (type: 'accept' | 'reject') => {
    const endpoint = type === 'accept' ? '/accept-request' : '/reject-request';

    try {
      const token = await AsyncStorage.getItem('access_token');
      const res = await fetch(
        `${process.env.EXPO_PUBLIC_API_URL}/friends${endpoint}`,
        {
          method: 'PATCH',
          headers: {
        ...
        <ScrollView>
          ...
          <View className="flex-row gap-4 mb-4">
            <Pressable
              ...
              className="border border-white px-6 py-4 rounded-lg items-center"
            >
